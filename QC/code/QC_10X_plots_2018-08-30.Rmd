...
### User input
Tell R the age of the dataset, the path to the `.RData` workspace file, and the name of the Seurat object contained within the workspace.
Also indicate the path to a metadata file with cell names and brain regions of origin.
```{r message=F}
age <- "GW18"

dataset <- "/kriegsteinlab/data1/aparna/homefiles/gw18.RData"

seurat.obj <- "gw18"

meta <- read_tsv("/kriegsteinlab/data1/carmen/2nd-trimester/metadata/colnames_GW18_080118_fixed.txt", 
                   col_names = c("cell.name", "brain.region"))
```

### Load `.RData` workspace and create a new Seurat object using the raw data.
...
ribogenes <- read_table("ribogenes.txt", col_names = F)$X1
...
p2 <- ggplot(seurat.obj.new@meta.data) + 
  ...
  scale_y_continuous(breaks=seq(0, nGene.limit, by=500), limits = c(0, nGene.limit)) +
  scale_x_discrete(breaks=(statsByArea %>% arrange(mean.nGene))$orig.ident) +
...
p4 <- 
  ...
  # ylim(0, 40000) +
  scale_y_continuous(breaks=seq(0, nUMI.limit, by=10000), limits=c(0,  nUMI.limit)) +
...
```{r print-plots}
png(paste0(age, "_QC_1.png"), width=25, height=16, units="in", res=120)
