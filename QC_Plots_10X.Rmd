...
### Set the brain region of cells as their original identity in the @ident slot and in the @meta.data slot.
```{r set-ident}
...
### Number and percentage of cells in each brain region.
```{r}
...
p5 <- ggplot(seurat.obj.new@meta.data %>% filter(nGene < nGene.limit)) + 
  ...
  # ylim(0, 40000) +
  scale_y_continuous(breaks=seq(0, nGene.limit, by=500), limits=c(0, nGene.limit)) +
  labs(title=paste0(age, ": Genes per Cell / UMIs per Cell"),
       x="", 
       y="Genes Detected",
       color= "")
...
```{r print-plots}
png(paste0(age, "_QC_1.png"), width=25, height=16, units="in", res=150)
